...
__all__ = [
    "key_list", "values_list", "value_data", "del_key", "del_value", "set_value", "key_exists"
]
...
class NoDataExistsException(BaseException):
    """Registry key does not contains any data or does not exists"""
    pass
...
def _root(string) -> bool:
    """REG_ROOTS -> Identify if there there is key in a string"""
...
def _enum(key, recurse=True) -> List[str]:
    """
...
def set_value(key, value, data, reg_type=None) -> bool:
    """
    Microsoft documentation on registry value types:
